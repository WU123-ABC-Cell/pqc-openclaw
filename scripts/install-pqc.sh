...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --skip-backup-timer) SKIP_BACKUP_TIMER=1; shift ;;
...
if [[ -z "$SANDBOX_ROOT" ]]; then
  ...
  chown -R "$SERVICE_USER" "$BACKUP_DIR" || die "failed to assign backup directory to $SERVICE_USER"
fi
...
# Generated by scripts/install-pqc.sh. Re-run the installer to update.

[Unit]
Description=Verified backup for PQC OpenClaw
After=pqc-openclaw.service
...
UMask=0077
SuccessExitStatus=2
ExecStart=$BIN_DIR/backup-pqc.sh --state-dir $STATE_DIR --backup-dir $BACKUP_DIR --healthcheck-bin $BIN_DIR/healthcheck-pqc.sh --healthcheck-skip-keyring --json
NoNewPrivileges=true
ProtectSystem=strict
ProtectHome=true
...
EOF
    ...
    cat > "$SYSTEMD_UNIT_DIR/pqc-openclaw-backup.timer" <<'EOF'
# Generated by scripts/install-pqc.sh. Re-run the installer to update.

[Unit]
Description=Daily verified backup for PQC OpenClaw

[Timer]
OnCalendar=*-*-* 03:00:00
RandomizedDelaySec=15m
Persistent=true
Unit=pqc-openclaw-backup.service

[Install]
WantedBy=timers.target
EOF
    ok "backup timer rendered at $SYSTEMD_UNIT_DIR/pqc-openclaw-backup.timer"

    if [[ -z "$SANDBOX_ROOT" ]]; then
      if [[ -d /run/systemd/system ]] && command -v systemctl >/dev/null 2>&1; then
        systemctl daemon-reload || die "systemd daemon-reload failed"
        systemctl enable --now pqc-openclaw-backup.timer || die "failed to enable backup timer"
        ok "pqc-openclaw-backup.timer enabled and active"
      else
        warn "systemd is not active; backup timer was rendered but could not be enabled"
      fi
    fi
  fi
fi
...
=========================================================================
  ...
  Backup timer: $SYSTEMD_UNIT_DIR/pqc-openclaw-backup.timer (rendered only)
...
NEXT STEPS
  ...
  3. Reload and start the service:
       sudo systemctl daemon-reload
    sudo systemctl enable --now pqc-openclaw
    sudo systemctl enable --now pqc-openclaw-backup.timer
