...
async function runExport(
  ...
  if (pw.length === 0) {
    return {
      exitCode: 2,
      stdout: "",
      stderr: "passphrase required (use --passphrase or pipe stdin)",
    };
  }
  const exportOpts: ExportOptions = iterations
    ? { passphrase: pw, iterations }
    : { passphrase: pw };
...
async function runImport(
  ...
  if (!blob)
    return {
      exitCode: 2,
      stdout: "",
      stderr: `usage: openclaw wrap-key import <blob>\n\n${USAGE}`,
    };
  const pw = await readPassphraseInteractive(ctx, "Passphrase: ", passphrase);
  if (pw.length === 0) {
    return {
  ...
  try {
    ...
    return {
      exitCode: 2,
      stdout: "",
      stderr: `invalid backup blob: ${err instanceof Error ? err.message : String(err)}`,
    };
  }
  ...
  try {
    if (hasImportKey(provider)) {
      ...
      return {
        exitCode: 0,
        stdout: `imported wrap key ${result.keyId} (became active: ${result.becameActive})\n`,
        stderr: "",
      };
    }
    ...
    return {
      exitCode: 0,
      stdout: `imported wrap key ${parsed.keyId} (provider does not persist; keep environment variable OPENCLAW_WRAP_KEY to use it)\n`,
      stderr: "",
    };
  } catch (err) {
...
async function runRotate(
  ...
  if (!confirm) {
    return {
      ...
      stdout: "",
      stderr:
        "refusing to rotate without --confirm: this rewraps every device identity in state.db.\nRun with --confirm to proceed.",
    };
  }
  const provider = getProvider(ctx);
...
async function runStatus(
  ...
  if (!probe.ok)
    return { exitCode: 1, stdout: "", stderr: `wrap-key probe failed: ${probe.error}` };
  const lines: string[] = [];
  lines.push("wrap-key status");
  lines.push(`  total device identities:  ${probe.probe.totalIdentities}`);
  lines.push(`  wrapped (PQC 2.2):       ${probe.probe.wrappedIdentities}`);
...
export async function runWrapKeyCommand(
  ...
  try {
    switch (sub) {
      case "export":
        return await runExport(rest, ctx);
      case "import":
        return await runImport(rest, ctx);
      case "rotate":
        return await runRotate(rest, ctx);
      case "status":
        return await runStatus(rest, ctx);
      default:
        return { exitCode: 2, stdout: "", stderr: `unknown subcommand: ${sub}\n\n${USAGE}` };
    }
  } catch (err) {
    return {
      exitCode: 1,
      stdout: "",
      stderr: `wrap-key ${sub} failed: ${err instanceof Error ? err.message : String(err)}`,
    };
  }
}
