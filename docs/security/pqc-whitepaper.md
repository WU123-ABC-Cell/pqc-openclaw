...
node pqc-fork-scripts/sidechannel-mldsa.mjs --algorithm ml_dsa65 --rounds 1500 --out sidechannel-mldsa-ml_dsa65-report.json
node pqc-fork-scripts/sidechannel-mldsa.mjs --algorithm ml_dsa87 --rounds 1500 --out sidechannel-mldsa-ml_dsa87-report.json
...
# cache-timing: 12 algos, 各自独立 cache-timing-ct-runner.sh 跑, 累计 ~2.5h
bash pqc-fork-scripts/cache-timing-ct-driver-all.sh 20 20
# 或单独跑一个 algo
bash pqc-fork-scripts/cache-timing-ct-runner.sh ml_dsa44  sign    20 20 ct-reports/ml_dsa44_sign
bash pqc-fork-scripts/cache-timing-ct-runner.sh ml_dsa44  verify  20 20 ct-reports/ml_dsa44_verify
...
bash pqc-fork-scripts/cache-timing-ct-runner.sh ml_dsa87  sign    20 20 ct-reports/ml_dsa87_sign
bash pqc-fork-scripts/cache-timing-ct-runner.sh ml_dsa87  verify  20 20 ct-reports/ml_dsa87_verify
bash pqc-fork-scripts/cache-timing-ct-runner.sh ml_kem512 encap   20 20 ct-reports/ml_kem512_encap
bash pqc-fork-scripts/cache-timing-ct-runner.sh ml_kem512 decap   20 20 ct-reports/ml_kem512_decap
...
- 旁路测试:
  ...
  - ✅ **已完成 (2026-08-27)**: dudect-style 跑 ML-DSA 全部 3 个 param set (44/65/87 sign/verify) — 18K ops, |t| < 1.8, 0 leak
  - ✅ **已完成 (2026-08-27)**: dudect-style 跑 ML-KEM 全部 3 个 param set (512/768/1024 encap/decap) — 60K ops, |t| < 1.5, 0 leak
  - ✅ **已完成 (2026-08-27/28)**: valgrind callgrind per-process aggregate cache-timing FIPS 203/204 全部 6 个 param set (12 ops) — 9.6K ops, max |t| < 2.0, 0 leak
  - ✅ **已完成 (2026-08-30)**: valgrind callgrind per-process aggregate cache-timing AES-256-GCM wrap/unwrap (2 ops, OpenSSL 3.x FIPS 140-3 path) — 1.6K ops, max |t|=1.808 [D1mr], 0 leak
  - valgrind callgrind **per-op** cache-timing via SIGUSR1/SIGUSR2 dump+zero (完整 cache-timing, P0, 14 algo × 5000 ops = 98h CPU)
